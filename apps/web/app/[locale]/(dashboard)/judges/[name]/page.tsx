...
export default async function JudgePage({ params, searchParams }: Props) {
  ...
  return (
    <div className="min-h-screen bg-premier-black text-white p-6 lg:p-10">
      <nav className="mb-6 text-sm text-zinc-500 flex items-center gap-2">
        ...
        <span>/</span>
        <span className="text-zinc-300">法官</span>
        <span>/</span>
        <span className="text-zinc-300">{judgeName}</span>
      </nav>

      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-1">{judgeName}</h1>
        ...
        {totalPages > 1 && (
          <div className="mt-8 flex items-center justify-center gap-3">
            {page > 1 && (
              <Link
                href={`/${locale}/judges/${encodeURIComponent(name)}?page=${page - 1}`}
