# Archive - Historical Documentation

This directory contains historical documentation for reference purposes.

> ⚠️ **Note**: These documents may be outdated. Refer to the main documentation for current information.

## Contents

### Implementation Documentation
- **[implementation-summary.md](./implementation-summary.md)** - Implementation completion notes (Feb 2026)
- **[nexus-integration.md](./nexus-integration.md)** - Nexus Legal integration process
- **[system-checks.md](./system-checks.md)** - System verification results

### Feature Implementation Archives
- **[fts-implementation.md](./fts-implementation.md)** - Full-text search implementation notes
- **[fulltext-search.md](./fulltext-search.md)** - Full-text search technical details
- **[rss-fixes.md](./rss-fixes.md)** - RSS feed troubleshooting history

### Deployment Archives
- **[deployment-summary.md](./deployment-summary.md)** - Historical deployment documentation
- **[deployment-ready.md](./deployment-ready.md)** - Deployment readiness checklist

## Why Archive?

These documents capture important implementation decisions and troubleshooting history, but are no longer part of the active documentation set. They have been preserved here for:

1. **Historical Reference** - Understanding past decisions and context
2. **Troubleshooting** - Learning from past issues and solutions
3. **Knowledge Transfer** - Onboarding new team members
4. **Audit Trail** - Tracking system evolution over time

## Current Documentation

For up-to-date information, please refer to:

- **Main Documentation**: [../README.md](../README.md)
- **Deployment Guide**: [../deployment/README.md](../deployment/README.md)
- **Development Guides**: [../guides/](../guides/)
- **Architecture**: [../ARCHITECTURE.md](../ARCHITECTURE.md)

---

## Document History

| Document | Original Date | Archived Date | Reason |
|----------|--------------|---------------|---------|
| implementation-summary.md | 2026-02-01 | 2026-02-09 | Implementation completed |
| nexus-integration.md | 2026-02 | 2026-02-09 | Integration completed |
| system-checks.md | 2026-02 | 2026-02-09 | One-time verification |
| fts-implementation.md | 2026-02 | 2026-02-09 | Feature completed |
| fulltext-search.md | 2026-02 | 2026-02-09 | Duplicate documentation |
| deployment-summary.md | 2026-02 | 2026-02-09 | Superseded by new guides |
| deployment-ready.md | 2026-02 | 2026-02-09 | Superseded by new guides |
| rss-fixes.md | 2026-02 | 2026-02-09 | Issues resolved |

---

**Note**: If you need to reference these documents regularly, they may need to be moved back to active documentation. Please open an issue to discuss.
